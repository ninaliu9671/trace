...
❌ OKR 目标管理
❌ 数据图表可视化
❌ 语音输入 / 文件上传（summary 的 AI 助手只支持图片）
❌ 付费订阅 / 数据导出
❌ 移动端 App（先做 Web）
```
...
**职业档案页例外说明：**
三个 Tab 各自有独立的 AI 助手上下文，但入口统一在右上角同一个按钮。切换 Tab 时按 6.3 的规则处理。
...
```
/profile · 职业画像 Tab → 读取：基本信息 + 工作职责 + 职业方向（已存储内容）
/profile · 汇报框架 Tab → 读取：完整汇报节点树
/profile · 记录维度 Tab → 读取：完整维度树结构
...
判断维度：**AI 对话里有没有未完成的事**（不看面板收起还是展开）

```
情况 A：AI 对话为空 / 已点击「结束对话」
  → 直接切换，不提示，自动加载新 Tab 上下文

情况 B：对话进行中，有未采纳的预览卡片
  → 弹提示：「你有一条 AI 整理结果还未采纳，切换后将丢失。」
  → [先采纳]  [放弃并切换]  [留在这里]

情况 C：对话进行中，无未采纳预览（纯对话状态）
  → 弹提示：「你和 AI 的对话还未结束，切换后对话记录将被清除。」
  → [确认切换]  [留在这里]
```

注意：切换 Tab 的判断逻辑同样适用于 /profile 的三个 Tab 之间的切换。
...
**职业档案 /profile：**
- AI 有专业主动性，不只顺从用户，会给出专业建议
- 根据当前 Tab 自动加载对应上下文
- 可以帮润色工作职责、梳理职业方向、优化汇报框架、调整记录维度
...
「✦ 和 AI 一起填写」：
  关闭弹窗 + 自动打开右侧 AI 助手
  AI 开场白（新用户）：
  「你好！我来帮你建立职业档案，整个过程大概 3–5 分钟。
   先告诉我——你目前的职位是什么，在哪个行业？」
...
```
你是职业顾问，帮用户完善职业档案。根据当前 Tab 切换任务：
...
```
阶段一：职业档案 /profile（含新用户引导）
├── ProfilePage.tsx             三 Tab 主页面 + AI 助手按钮
├── OnboardingModal.tsx         新用户引导弹窗（集成在此页面）
├── AiSidePanel.tsx             右侧 AI 对话栏（通用，所有页面复用）
├── BasicInfoCard.tsx           基本信息（含编辑态 + 空状态）
├── ResponsibilitiesCard.tsx    工作职责（含空状态提示）
├── CareerDirectionCard.tsx     职业方向（含空状态 + 技能标签）
├── ReportNodeTree.tsx          汇报框架树（含空状态虚线卡片）
├── ReportNodeEditor.tsx        汇报节点编辑表单
├── DimensionTree.tsx           记录维度树（含空状态虚线卡片）
└── DimensionNodeEditor.tsx     维度节点编辑

阶段二：工作日志 /log
├── LogPage.tsx                 两栏主布局 + AI 助手按钮
├── DateNav.tsx                 顶部日期导航（箭头 + 日历弹出）
├── DimensionDirectory.tsx      左栏目录（含状态圆点指示）
├── LogContent.tsx              右栏内容区（提示词 + 输入框）
├── LogField.tsx                单个字段（含手动/AI填入两种状态）
└── SaveButton.tsx              底部保存按钮（含 loading/saved 状态）

阶段三：汇报总结 /summary
├── SummaryPage.tsx             主页面（左列表 + 右内容区）
├── SummaryList.tsx             左侧历史列表（按月分组）
├── SummaryListItem.tsx         单条总结（含文档属性浮层）
├── NewSummaryModal.tsx         新建总结弹窗
├── DataCompletenessAlert.tsx   数据完整度提示弹窗
├── SummaryTopbar.tsx           文档操作栏（恢复/模式切换/AI/定稿）
├── MarkdownEditor.tsx          编辑/预览切换编辑器
...
```
1. 每次只做一个组件，做完测试通过再做下一个
2. API 调用统一放在 /app/api/，组件不直接调外部 API
3. 所有 Prompt 写在 /lib/prompts.ts
4. 环境变量放 .env.local，不硬编码
5. 每个 API 路由必须有 try/catch，出错返回友好提示
6. 每个等待操作必须有 loading 状态
7. 新依赖先告知，不擅自引入
8. 所有数据结构在 /types/index.ts 定义 interface
9. 不做文档里没有要求的功能
10. 每个页面必须在 375px 手机屏幕能正常使用
11. AiSidePanel.tsx 是通用组件，/profile、/log、/summary 都复用它
    不要为每个页面单独写一个 AI 面板
12. OnboardingModal.tsx 只在 /profile 检测到新用户时渲染
    不要做独立的 /onboarding 路由或页面
```
...
3. AI 问职位、汇报周期等信息（5–8 轮对话）
4. AI 生成汇报框架 + 记录维度，展示预览让用户确认
5. 用户确认 → 数据写入 Supabase → 三个 Tab 显示已填内容
6. 底部出现「前往工作日志 →」跳转按钮
7. 数据库 report_nodes 和 dimensions 表能查到对应记录
...
*文档版本：v5.0 | 2026.04.24 | 整合 onboarding 到 profile + 统一导航名称（职业档案/工作日志/汇报总结）+ summary 完整设计 + AI 助手完整规范*
